...
function check_project_struct {
  ...
  find_file_or_dir_in_project ${PROJECT_PATH} data
...
function setup_testing_input_output {
  ...
  cp -r ${PROJECT_PATH}/data ${TEST_OUTPUT_PATH}
  cp -r ${PROJECT_PATH}/output ${TEST_OUTPUT_PATH}

  rm -r ${TEST_OUTPUT_PATH}/data/*
  rm -r ${TEST_OUTPUT_PATH}/output/*
  cp -r ${GRADER_ROOT}/tests/${test_folder}/data/tweets.txt ${TEST_OUTPUT_PATH}/data/tweets.txt
}
